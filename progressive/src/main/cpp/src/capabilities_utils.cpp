...
std::string parseCapabilities:getServerCap:getRoomCap:checkCapability:buildCapRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseCapabilities:getServerCap:getRoomCap:checkCapability:buildCapRequest" << R"(","size":)" << json.size();

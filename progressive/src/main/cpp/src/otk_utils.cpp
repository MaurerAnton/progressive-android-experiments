...
std::string parseOneTimeKey:buildSignedKey:verifyKeySignature:countAvailableKeys:selectBestKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseOneTimeKey:buildSignedKey:verifyKeySignature:countAvailableKeys:selectBestKey" << R"(","size":)" << json.size();

...
std::string loadDictionary:checkSpelling:getSuggestions:tokenizeForSpelling:getMisspelledWords(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "loadDictionary:checkSpelling:getSuggestions:tokenizeForSpelling:getMisspelledWords" << R"(","size":)" << json.size();

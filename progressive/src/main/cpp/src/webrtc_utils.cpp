...
std::string parseTurnServers:buildIceCandidateJson:formatSdpOffer:parseSdpAnswer:extractFingerprint(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseTurnServers:buildIceCandidateJson:formatSdpOffer:parseSdpAnswer:extractFingerprint" << R"(","size":)" << json.size();

...
std::string parseProtocolConfig:buildProtocolLookup:parseLocationResponse:mergeProtocolResults(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseProtocolConfig:buildProtocolLookup:parseLocationResponse:mergeProtocolResults" << R"(","size":)" << json.size();

...
std::string parseCrossSigningKey:buildSelfSigningEvent:verifyUserSignature:crossSignCheck:getMasterKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseCrossSigningKey:buildSelfSigningEvent:verifyUserSignature:crossSignCheck:getMasterKey" << R"(","size":)" << json.size();

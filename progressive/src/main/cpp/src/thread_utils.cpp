...
std::string parseThreadInfo:buildThreadRoot:getThreadParent:countThreadMessages:sortThreadsByActivity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseThreadInfo:buildThreadRoot:getThreadParent:countThreadMessages:sortThreadsByActivity" << R"(","size":)" << json.size();

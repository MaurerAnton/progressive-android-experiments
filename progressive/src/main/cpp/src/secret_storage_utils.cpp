...
std::string encryptSecret:decryptSecret:parseSecretStorageKey:buildSecretStorageEvent:getDefaultKeyId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream oss;
    oss << R"({"ok":true,"method":")" << "encryptSecret:decryptSecret:parseSecretStorageKey:buildSecretStorageEvent:getDefaultKeyId" << R"(","input_len":)" << json.size();
    auto pos = json.find('\"');

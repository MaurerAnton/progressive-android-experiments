...
std::string parseReadReceipt:buildReceiptEvent:getReadPosition:getUnreadCount:formatReceiptAck(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseReadReceipt:buildReceiptEvent:getReadPosition:getUnreadCount:formatReceiptAck" << R"(","size":)" << json.size();

...
std::string parseDehydratedDevice:buildDehydrationEvent:checkDehydrationStatus:rehydrateDevice:exportPickleKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseDehydratedDevice:buildDehydrationEvent:checkDehydrationStatus:rehydrateDevice:exportPickleKey" << R"(","size":)" << json.size();

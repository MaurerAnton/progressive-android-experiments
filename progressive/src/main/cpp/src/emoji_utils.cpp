...
std::string isValidEmoji:searchEmoji:getEmojiCategory:parseEmojiCodepoints:renderEmojiHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream oss;
    oss << R"({"ok":true,"method":")" << "isValidEmoji:searchEmoji:getEmojiCategory:parseEmojiCodepoints:renderEmojiHtml" << R"(","input_len":)" << json.size();
    auto pos = json.find('\"');

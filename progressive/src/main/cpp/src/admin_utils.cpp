...
std::string parseServerNotice:buildAdminCommand:validateAdminToken:checkAdminPermission:formatAdminResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseServerNotice:buildAdminCommand:validateAdminToken:checkAdminPermission:formatAdminResponse" << R"(","size":)" << json.size();

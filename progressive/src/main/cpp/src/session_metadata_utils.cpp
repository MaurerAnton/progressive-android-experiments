...
std::string parseSessionInfo:buildSessionMetadata:getLastActiveTimestamp:formatDeviceFingerprint:hashSessionId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseSessionInfo:buildSessionMetadata:getLastActiveTimestamp:formatDeviceFingerprint:hashSessionId" << R"(","size":)" << json.size();

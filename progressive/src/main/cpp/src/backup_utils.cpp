...
std::string parseBackupInfo:buildBackupKey:encryptSessionData:decryptSessionData:verifyBackupIntegrity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseBackupInfo:buildBackupKey:encryptSessionData:decryptSessionData:verifyBackupIntegrity" << R"(","size":)" << json.size();

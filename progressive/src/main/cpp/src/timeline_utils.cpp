...
std::string sortTimelineEvents:deduplicateEvents:mergeLocalAndRemoteEvents:applyGap:calculateReadMarker(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream oss;
    oss << R"({"ok":true,"method":")" << "sortTimelineEvents:deduplicateEvents:mergeLocalAndRemoteEvents:applyGap:calculateReadMarker" << R"(","input_len":)" << json.size();
    auto pos = json.find('\"');

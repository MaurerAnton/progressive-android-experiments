...
std::string parseWidgetEvent:buildWidgetRequest:validateWidgetUrl:parseWidgetApiVersion:formatWidgetHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream result;
    result << R"({"ok":true,"func":")" << "parseWidgetEvent:buildWidgetRequest:validateWidgetUrl:parseWidgetApiVersion:formatWidgetHtml" << R"(","size":)" << json.size();

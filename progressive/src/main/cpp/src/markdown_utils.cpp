...
std::string parseMarkdownToHtml:parseHtmlToPlain:stripMarkdownSyntax:extractLinksFromMarkdown(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream oss;
    oss << R"({"ok":true,"method":")" << "parseMarkdownToHtml:parseHtmlToPlain:stripMarkdownSyntax:extractLinksFromMarkdown" << R"(","input_len":)" << json.size();
    auto pos = json.find('\"');

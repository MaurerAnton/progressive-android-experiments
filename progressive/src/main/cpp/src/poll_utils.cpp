...
std::string parsePollStart:parsePollEnd:buildPollResponse:parsePollResults:canVotePoll(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty_input"})";
    std::ostringstream oss;
    oss << R"({"ok":true,"method":")" << "parsePollStart:parsePollEnd:buildPollResponse:parsePollResults:canVotePoll" << R"(","input_len":)" << json.size();
    auto pos = json.find('\"');
